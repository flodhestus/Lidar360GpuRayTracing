...
sensor_msgs_msg_Image* FLidar360Dds::AllocImageSample()
{
	...
	Sample->data._maximum = LIDAR360_IMAGE_BYTES;
	Sample->data._buffer = static_cast<uint8_t*>(dds_alloc(LIDAR360_IMAGE_BYTES));
	Sample->data._release = true;
	return Sample;
#else
	return nullptr;
...
bool FLidar360Dds::TakeLatestImage(int32 Reader, FLidar360ImageFrame& OutFrame)
{
#if WITH_LIDAR360_DDS
	if (Reader <= 0)
	{
