...
class FLidar360CameraCaptureCS : public FGlobalShader
{
public:
	DECLARE_GLOBAL_SHADER(FLidar360CameraCaptureCS);
	SHADER_USE_PARAMETER_STRUCT(FLidar360CameraCaptureCS, FGlobalShader);
...
IMPLEMENT_GLOBAL_SHADER(
	FLidar360CameraCaptureCS,
	"/Lidar360GpuRayTracingShaders/Lidar360CameraCapture.usf",
	"Lidar360CameraCaptureCS",
	SF_Compute);
...
bool FLidar360CameraCapture::CaptureRgb8(
	...
	int32& OutHeight)
{
	...
	TShaderMapRef<FLidar360CameraCaptureCS> ComputeShader(GetGlobalShaderMap(GMaxRHIFeatureLevel));
	...
	ENQUEUE_RENDER_COMMAND(Lidar360CameraCapture)(
		[Resource, &Packed, OutWidth, OutHeight, ComputeShader, Done](FRHICommandListImmediate& RHICmdList)
		{
			...
			FRDGTextureRef InputTexture = GraphBuilder.RegisterExternalTexture(
				CreateRenderTarget(Resource->GetRenderTargetTexture(), TEXT("Lidar360CameraInput")));
			FRDGBufferRef OutputBuffer = GraphBuilder.CreateBuffer(
				FRDGBufferDesc::CreateStructuredDesc(sizeof(uint32), static_cast<uint32>(OutWidth * OutHeight)),
				TEXT("Lidar360CameraPacked"));

			FLidar360CameraCaptureCS::FParameters* Params = GraphBuilder.AllocParameters<FLidar360CameraCaptureCS::FParameters>();
			...
			FComputeShaderUtils::AddPass(
				GraphBuilder,
				RDG_EVENT_NAME("Lidar360CameraCaptureCS"),
				ComputeShader,
				Params,
				FIntVector(
					FMath::DivideAndRoundUp(OutWidth, 8),
					FMath::DivideAndRoundUp(OutHeight, 8),
					1));

			FRHIGPUBufferReadback* Readback = new FRHIGPUBufferReadback(TEXT("Lidar360CameraReadback"));
