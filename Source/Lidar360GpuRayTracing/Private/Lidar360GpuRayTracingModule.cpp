...
static void SpawnCameraPublisher(UWorld* World)
{
	if (!World || World->WorldType != EWorldType::PIE)
	{
		return;
	}
	for (TActorIterator<ALidar360CameraPublisher> It(World); It; ++It)
	{
		return;
	}
	World->SpawnActor<ALidar360CameraPublisher>();
}
...
void FLidar360GpuRayTracingModule::StartupModule()
{
	...
	FWorldDelegates::OnPIEStarted.AddLambda([](const bool)
	{
		...
		for (const FWorldContext& Ctx : GEngine->GetWorldContexts())
		{
			...
			{
				SpawnCameraPublisher(Ctx.World());
			}
		}
	});
#if RHI_RAYTRACING
